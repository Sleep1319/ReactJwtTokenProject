package com.apiboad6.reactjwttokenproject.controller.signController;
...
public class SignController {
    ...
    public ResponseEntity<?> signIn(@Valid @RequestBody SignInRequest req, HttpSession session) {
        SignInResponse res = signService.signIn(req);
        session.setAttribute("user", res);

        log.info("세션에 저장된 정보 {}", session.getAttribute("user"));
